...
export default function ToolContent() {
  ...
  const getWinnerStyle = (winner: number, side: number) => {
    if (winner === side) return { color: "#166534", fontWeight: 800 as const };
      ...
      <div style={{ background: "rgba(10,30,94,0.1)", border: "1px solid rgba(10,30,94,0.12)", borderRadius: 14, padding: 24 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button
            onClick={analyze}
            disabled={!canSubmit}
            style={{ padding: "12px 24px", borderRadius: 8, border: "none", background: canSubmit ? "#0a1e5e" : "rgba(10,30,94,0.15)", color: canSubmit ? "#0a1e5e" : "rgba(10,30,94,0.4)", fontSize: 13, fontFamily: "'Syne', sans-serif", fontWeight: 700, cursor: canSubmit ? "pointer" : "not-allowed", letterSpacing: 0.5, whiteSpace: "nowrap" }}
          >
        ...
        {result?.success && result.account1 && result.account2 && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 1fr", gap: 0, marginBottom: 16 }}>
              <div style={{ background: "rgba(10,30,94,0.1)", border: "1px solid rgba(10,30,94,0.12)", borderRadius: "12px 0 0 12px", padding: 20, textAlign: "center" }}>
                <div style={{ fontSize: 18, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#0a1e5e" }}>@{result.account1.handle}</div>
              ...
              <div style={{ background: "rgba(10,30,94,0.1)", borderTop: "1px solid rgba(10,30,94,0.12)", borderBottom: "1px solid rgba(10,30,94,0.12)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: 14, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "rgba(10,30,94,0.5)" }}>VS</span>
              </div>
              <div style={{ background: "rgba(10,30,94,0.1)", border: "1px solid rgba(10,30,94,0.12)", borderRadius: "0 12px 12px 0", padding: 20, textAlign: "center" }}>
                <div style={{ fontSize: 18, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#0a1e5e" }}>@{result.account2.handle}</div>
            ...
            {result.comparisons && result.comparisons.map((row: any, i: number) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 120px 1fr", gap: 0, marginBottom: 2 }}>
                <div style={{ background: "rgba(10,30,94,0.1)", border: "1px solid rgba(10,30,94,0.12)", borderRadius: i === 0 ? "12px 0 0 0" : i === result.comparisons.length - 1 ? "0 0 0 12px" : 0, padding: "14px 18px", textAlign: "right" }}>
                  <span style={{ fontSize: 16, fontFamily: "'Syne', sans-serif", ...getWinnerStyle(row.winner, 1) }}>
                    {row.value1}
                  </span>
                  {row.winner === 1 && (
                    <span style={{ marginLeft: 8, fontSize: 9, fontFamily: "'DM Mono', monospace", padding: "2px 6px", borderRadius: 8, background: "rgba(34,197,94,0.12)", color: "#166534", border: "1px solid rgba(34,197,94,0.25)", textTransform: "uppercase", letterSpacing: 1 }}>WIN</span>
                  )}
                </div>
                <div style={{ background: "rgba(10,30,94,0.1)", borderTop: "1px solid rgba(10,30,94,0.12)", borderBottom: "1px solid rgba(10,30,94,0.12)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                <div style={{ background: "rgba(10,30,94,0.1)", border: "1px solid rgba(10,30,94,0.12)", borderRadius: i === 0 ? "0 12px 0 0" : i === result.comparisons.length - 1 ? "0 0 12px 0" : 0, padding: "14px 18px", textAlign: "left" }}>
                  {row.winner === 2 && (
                    <span style={{ marginRight: 8, fontSize: 9, fontFamily: "'DM Mono', monospace", padding: "2px 6px", borderRadius: 8, background: "rgba(34,197,94,0.12)", color: "#166534", border: "1px solid rgba(34,197,94,0.25)", textTransform: "uppercase", letterSpacing: 1 }}>WIN</span>
                  )}
            ...
            {result.overallWinner && (
              <div style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 12, padding: 20, textAlign: "center", marginTop: 16 }}>
                <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#166534", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>Overall Winner</div>
                <div style={{ fontSize: 24, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#166534" }}>@{result.overallWinner}</div>
