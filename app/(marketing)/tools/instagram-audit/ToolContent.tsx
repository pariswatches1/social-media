    ...
    >
      ...
      <ToolResults loading={loading} error={error} dataSource={result?.dataSource}>
        {result?.success && (
          <div>
            {/* Health Score */}
            <div style={{ background: "#0a0d14", border: "1px solid #1e2535", borderRadius: 12, padding: 32, textAlign: "center", marginBottom: 16 }}>
              <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#4a5568", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Account Health Score</div>
              <div style={{ position: "relative", width: 140, height: 140, margin: "0 auto 12px" }}>
                <svg viewBox="0 0 140 140" style={{ transform: "rotate(-90deg)" }}>
                  <circle cx="70" cy="70" r="60" fill="none" stroke="#1e2535" strokeWidth="10" />
                ...
                <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)" }}>
                  ...
                  <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#4a5568" }}>/100</div>
                </div>
              </div>
            </div>
            ...
            {result.metrics && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
                {result.metrics.map((metric: any) => (
                  <div key={metric.label} style={{ background: "#0a0d14", border: "1px solid #1e2535", borderRadius: 12, padding: 16, textAlign: "center" }}>
                    <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#4a5568", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>{metric.label}</div>
                    <div style={{ fontSize: 22, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#e2e8f0" }}>{metric.value}</div>
                  </div>
                ))}
              </div>
            )}
                ...
                <div style={{ background: "#0a0d14", border: "1px solid #1e2535", borderRadius: 12, padding: 20 }}>
                  ...
                  {result.strengths.map((item: string, i: number) => (
                    <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 10 }}>
                      ...
                      <span style={{ fontSize: 13, fontFamily: "'DM Sans', sans-serif", color: "#e2e8f0", lineHeight: "20px" }}>{item}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div style={{ background: "#0a0d14", border: "1px solid #1e2535", borderRadius: 12, padding: 20 }}>
                ...
                {result.recommendations.map((rec: string, i: number) => (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 12, padding: 12, borderRadius: 8, background: "rgba(6,182,212,0.04)", border: "1px solid rgba(6,182,212,0.1)" }}>
                    ...
                    <span style={{ fontSize: 13, fontFamily: "'DM Sans', sans-serif", color: "#e2e8f0", lineHeight: "20px" }}>{rec}</span>
                  </div>
                ))}
              </div>
            )}
