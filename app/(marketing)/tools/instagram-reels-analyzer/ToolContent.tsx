          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ background: "rgba(10,30,94,0.1)", border: "1px solid rgba(10,30,94,0.12)", borderRadius: 12, padding: 18, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#0a1e5e" }}>{result.reelsPercentage?.toFixed(1) ?? "—"}%</div>
              </div>
              <div style={{ background: "rgba(10,30,94,0.1)", border: "1px solid rgba(10,30,94,0.12)", borderRadius: 12, padding: 18, textAlign: "center" }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#166534" }}>{result.avgPlayCount?.toLocaleString() ?? "—"}</div>
              </div>
            </div>
            ...
            <div style={{ background: "rgba(10,30,94,0.1)", border: "1px solid rgba(10,30,94,0.12)", borderRadius: 12, padding: 20, marginBottom: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div style={{ padding: 16, borderRadius: 10, background: "rgba(6,182,212,0.06)", border: "1px solid rgba(6,182,212,0.15)", textAlign: "center" }}>
                  <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#0a1e5e", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>Reels Engagement</div>
                  <div style={{ fontSize: 32, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#0a1e5e" }}>{result.reelsEngagement?.toFixed(2) ?? "—"}%</div>
                </div>
              ...
              {result.reelsEngagement != null && result.overallEngagement != null && (
                <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 8, background: result.reelsEngagement > result.overallEngagement ? "rgba(34,197,94,0.06)" : "rgba(245,158,11,0.06)", border: `1px solid ${result.reelsEngagement > result.overallEngagement ? "rgba(34,197,94,0.15)" : "rgba(245,158,11,0.15)"}`, textAlign: "center" }}>
                  <span style={{ fontSize: 13, fontFamily: "'DM Sans', sans-serif", color: result.reelsEngagement > result.overallEngagement ? "#166534" : "#b45309" }}>
                ...
                {result.topReels.slice(0, 3).map((reel: any, i: number) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: i < 2 ? "1px solid rgba(255,255,255,0.3)" : "none" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, borderRadius: 8, background: "rgba(6,182,212,0.1)", border: "1px solid rgba(6,182,212,0.2)", fontSize: 14, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#0a1e5e" }}>#{i + 1}</span>
                      <div>
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ display: "flex", gap: 10, marginTop: 2 }}>
                        <span style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#166534" }}>{reel.likes?.toLocaleString()} likes</span>
                        <span style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#0a1e5e" }}>{reel.comments?.toLocaleString()} comments</span>
                      </div>
                    </div>
                  </div>
                ))}
