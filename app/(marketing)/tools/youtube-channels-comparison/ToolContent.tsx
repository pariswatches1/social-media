          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              {[result.channel1, result.channel2].map((ch: any, idx: number) => (
                <div key={idx} style={{ background: "#0a0d14", border: `1px solid ${result.comparison?.overallWinner === ch.name ? "#FF0000" : "#1e2535"}`, borderRadius: 12, padding: 20 }}>
                  ...
                  <h3 style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 700, color: "#fff", margin: "0 0 14px" }}>{ch.name}</h3>
                  {[
                    ...
                    <div key={stat.label} style={{ marginBottom: 12 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#4a5568", letterSpacing: 2, textTransform: "uppercase" }}>{stat.label}</span>
                        {stat.winner && (
                      ...
                      <div style={{ fontSize: 20, fontWeight: 800, fontFamily: "'Syne', sans-serif", color: "#e2e8f0", marginTop: 4 }}>{stat.value}</div>
                    </div>
                  ))}
                  <div style={{ marginTop: 8 }}>
                    <span style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#4a5568", letterSpacing: 2, textTransform: "uppercase" }}>Content Style</span>
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#94a3b8", margin: "4px 0 0" }}>{ch.contentStyle}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {[
              ...
              <div key={item.label} style={{ background: "#0a0d14", border: "1px solid #1e2535", borderRadius: 12, padding: 20, marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#4a5568", letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>{item.label}</div>
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#e2e8f0", margin: 0, lineHeight: 1.6 }}>{item.value}</p>
              </div>
            ))}
